...
annotation, database, MSigDB XML or Seurat RDS file, and provide a name for the
application to use. Options below are specified for annotation and database
files, including which file delimiter to use, whether the file has a header as
well as which columns to use for content and descriptions.
...
![Bar plot pane](../man/figures/bars_pane.png)

Shown above/below is the **bar plot pane**. Here, there are options for
...
![Overlap plot pane](../man/figures/overlap_pane.png)

Shown above/below is the **overlap plot pane**. Here, there are options for
configuring the **Overlap** output, including the coloring and scale adjustment
of the boxes. The plot will omit boxes with NA, NaN, Inf and -Inf values, so
the "Gene Value", "Odds Ratio", "Fold Enrichment" and "Adjusted Fold Enrichment"
options should be used with caution.

Below this are sliders for selecting annotations and genes, limited to 80 and
100 respectively. Note that the gene slider is not displayed; this will be shown
when genes are entered as input.

# Seurat data pane

![Seurat data pane](../man/figures/cell_pane.png)
...
to upload. Additionally, there is an option to downsample the Seurat data set. A
low value is recommended for low-memory systems.
...
the **advanced pane**.
* **Contents** displays the list of all genes from the database and Seurat files
respectively.
* **Recognised** splits genes from the input into two lists based on whether
they were found in the database.
* **Mismatched** displays which gene sets from the annotation file were not
found in the database file, and vice-versa.
* **Gene Values** splits genes from the input into two lists based on whether
they were assigned values.
